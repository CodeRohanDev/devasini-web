...
import { coreLeadership, LeaderProfile } from '@/lib/leadership-data'
import PageHero from '@/components/ui/PageHero'

export default function CoreLeaderClient({ leader }: { leader: LeaderProfile }) {
    return (
        <>
            <PageHero
                title={leader.name}
                ...
                breadcrumb={[
                    ...
                    { label: leader.name, href: `/leadership/core/${leader.slug}` },
                ]}
            />
            ...
            <section className="section-padding bg-navy-50">
                <div className="container-custom">
                    <h2 className="heading-3 mb-8">Other Core Leadership</h2>
                    <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
                        {coreLeadership
                            ...
                            .map((member) => (
                                <Link
                                    key={member.slug}
                                    href={`/leadership/core/${member.slug}`}
                                    className="group"
                                >
                                    <div className="aspect-[4/5] bg-gradient-to-br from-navy-100 to-navy-200 rounded-lg mb-4 overflow-hidden relative">
                                        {member.image ? (
                                            <img
                                                src={member.image}
                                                alt={member.name}
                                            ...
                                            <div className="absolute inset-0 flex items-center justify-center">
                                                <div className="w-20 h-20 bg-navy-300 rounded-full" />
                                            </div>
                                        )}
                                    ...
                                    <p className="text-sm text-navy-500">{member.title}</p>
                                </Link>
                            ))}
                    </div>
                </div>
